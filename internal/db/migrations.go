...
func NewMigrator(ctx context.Context, connParams ConfigProvider, fs embed.FS) (migrator DBMigrator, fault error) {
	conn, err := pgx.Connect(ctx, connParams.GetURI())
	if err != nil {
		return DBMigrator{}, fmt.Errorf("could not connect to database '%s' : %w", connParams.GetRedactedURI(), err)
	}
...
func RunMigrations(ctx context.Context, connParams ConfigProvider, fs embed.FS, stopAfter int32, printSummary bool) (fault error) {
	...
	if printSummary {
		fmt.Printf("Migrations for %s:%s/%s\n", info.Hostname, info.Port, info.Database)
