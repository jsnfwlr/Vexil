...
func TestValidateMigrations(t *testing.T) {
	...
	mFiles, err := etc.Migrations.ReadDir(".")
	if err != nil {
...
/*
func TestJustMigrations(t *testing.T) {
	if os.Getenv("BUILDKITE") == "true" {
		t.Skip("dev-only test - skipping test in CI environment")
	}
	...
	t.Setenv("POSTGRES_DATABASE", "vexil")
	t.Setenv("POSTGRES_USER", "vexil")
	t.Setenv("POSTGRES_PASSWORD", "vexil")
	...
	mig, err := migrations.NewMigrator(ctx, cfg, db.Migrations)
	if err != nil {
		t.Fatalf("error initialising migrator: %s", err)
	}

	current, target, _, err := mig.Info()
	if err != nil {
		t.Fatalf("error getting migration info: %s", err)
	}
	...
	t.Run("migrations", func(t *testing.T) {
		for i := current; i <= target; i++ {
			t.Run(fmt.Sprintf("%03d", i), func(t *testing.T) {
				err = mig.MigrateTo(i)
				if err != nil {
					t.Fatalf("error performing migration #%d: %s", i, err)
				}
			})
		}
	})
